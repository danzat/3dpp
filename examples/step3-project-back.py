...
m = intersect((a, b), (c, d))
scene.line(V, V + 3 * (m - V), style='red!30')

alpha = ((A - V) ^ (B - A)).length / ((m - V) ^ (B - A)).length

Mab = V + alpha * (m - V)

scene.point(Mab, style=f'circle, fill, color=red, inner sep=0pt, minimum size=3pt')
scene.label(Mab, r'$\vec{M}_\mathbf{AB}$', style='DarkOrchid, anchor=north west')

beta = ((C - V) ^ (D - C)).length / ((m - V) ^ (D - C)).length

Mcd = V + beta * (m - V)

scene.point(Mcd, style=f'circle, fill, color=red, inner sep=0pt, minimum size=3pt')
scene.label(Mcd, r'$\vec{M}_\mathbf{CD}$', style='OliveGreen, anchor=south west')
